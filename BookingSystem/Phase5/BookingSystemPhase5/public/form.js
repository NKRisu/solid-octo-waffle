...
function buildValidationMessage(errors) {
  ...
  */
  // Clear information of what is allowed in the fields with no error codes or variable names.
  return `Resource name field can only contain letters, numbers and spaces. Reservation creation failed.`; //\n\n${lines.join("\n")
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        showFormMessage("info", `${msg}`); // removed the error code, above line is clear for user.
        return;
      }
    ...
    const msgLines = [];
    /*
    Really helpful logs for developer, not helpful for user, changed to simple "Reservation creation succesful!"
    ...
    */
    msgLines.push("Reservation creation succesful!");
