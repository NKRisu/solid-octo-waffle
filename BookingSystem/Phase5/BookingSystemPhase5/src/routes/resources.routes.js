...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    // NOTE: Logging is currently using simple hard-coded messages for clarity.
    // Could be improved with more structured logging system with:
    //  standardized event types (CREATE_SUCCESS, CREATE_DUPLICATE, etc.)
    //  automatic formatting inside logEvent() helper function
    // for now though, these should be clear enough..

    const resourceId = rows[0].id;
    await logEvent({
      actorUserId,
      message: `Resource created succesfully: ${resourceName} (id=${resourceId})`, // clear success message with name and id
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `Failed to create resource: ${resourceName}, duplicate name`, // clear duplicate error message
        entityType: "resource",
        entityId: null,
      });
