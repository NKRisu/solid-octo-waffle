...
// added the other buttons too for clarity, if they are used later on in something. right now they arent so...
let createButton = null;
let updateButton = null;
let deleteButton = null;

// Resource name, description, price and availability validation status (these didnt have semicolon?)
let resourceNameValid = false;
let resourceDescriptionValid = false;
let resourcePriceValid = false;
let resourceAvailabilityValid = false;
...
// Adding the resource availability validation check, was not in the version 3 file but was required in version 2?
function isResourceAvailabilityValid(input) { return input.checked === true; }

// Adding the resource price validation check, was not in the version 3 file but was required in version 2?
...
// singular updater function in preparations to clearing the attachement functions. Which i might do when I am no longer sick.
function updateCreateButtonState() {
  setButtonEnabled(
    createButton,
    resourceNameValid &&
    resourceDescriptionValid &&
    resourcePriceValid &&
    resourceAvailabilityValid
  );
}
...
// Attach price.. (lot of copy paste code, should combine these functions into one)
function attachResourcePriceValidation(input) {
  const update = () => {
    ...
    resourcePriceValid = isResourcePriceValid(raw);

    setInputVisualState(input, resourcePriceValid ? "valid" : "invalid");
...
// Really should combine these functions...
function attachResourceAvailabilityValidation(input) {
  const update = () => {
    resourceAvailabilityValid = isResourceAvailabilityValid(input);
...
const resourcePriceInput = resourcePrice;
attachResourcePriceValidation(resourcePriceInput);
const resourceAvailabilityInput = resourceAvailable;
attachResourceAvailabilityValidation(resourceAvailabilityInput);
